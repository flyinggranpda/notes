...
> 📝 **文档摘要：** 本文聚焦 RAG 实战代码，涵盖元数据检索、查询扩展（Multi-Query）和 RRF 融合排序三大核心技术。元数据检索部分讲解如何让 LLM 拆解问题中的搜索关键词和过滤条件，先在元数据层面筛选再做向量检索。查询扩展部分比较 LangChain 内置 MultiQueryRetriever 与手写实现的差异，通过生成多个问题变体分别检索来提升召回率。RRF 融合排序部分从原理公式推导到手工演算再到完整代码实现，详细讲解 `score = Σ 1/(rank + k)` 的数学逻辑和 k 值调参经验（默认 k=60）。最后整合成一个完整的 MultiQueryRRFEngine 引擎，并提供 LLM 在检索流程中的多位置应用方式对比。
