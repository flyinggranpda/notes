...
> 💡 线性回归本质上就是 `Y = XW + B`（或 `Y = WX + B` 的数学写法），而神经网络的一个全连接层（Linear Layer）做的也是 `Y = XWᵀ + B`。所以用一个**单层神经网络**就能实现线性回归！
...
# 预设真实参数（这是"本质规律"）
w_real = torch.tensor([[2.5]])   # 斜率 ⭐ (1,1) ← 和 X 形状匹配
b_real = torch.tensor([[5.2]])   # 截距 ⭐ (1,1) ← 和 X 形状匹配
...
> 💡 `nn.Linear(1, 1)` 表示输入1个特征，输出1个值。内部自动创建了权重 `W`（1×1）和偏置 `B`（1），这正是 `Y = XW + B`。
...
│
├── ① 准备数据
│   ├── 生成样本：X = randn, Y = XW + B + noise
│   ├── TensorDataset：打包 X,Y
│   └── DataLoader：自动分批 + 打乱
│
├── ② 构建模型
│   ├── nn.Linear(1, 1) → Y = XW + B
│   └── 一个全连接层就能实现线性回归
│
├── ③ 定义损失和优化器
│   ├── nn.MSELoss() → 回归损失
...
# ============ 1. 准备数据 ============
X = torch.randn(100, 1)
w_real, b_real = torch.tensor([[2.5]]), torch.tensor([[5.2]])  # ⭐ (1,1) 形状
noise = torch.randn(100, 1) * 0.1
Y = X @ w_real + b_real + noise
