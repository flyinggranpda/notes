...
> 📝 **文档摘要：** 本文讲解 RAG 系统的完整评估体系。从"没有评估 = 靠感觉"的问题出发，引出四个核心评估指标：精确度（检索到的文档中真正有用的比例）、召回率（所有相关文档中被检索引擎找到的比例）、忠诚度（大模型回答是否基于检索到的文档而非瞎编）、答案相关性（回答是否解决用户问题）。每个指标都有详细的公式推导、计算公式、代码实现和优化方向。然后讲解如何构建评估数据集（RAGEvaluationDataset），以及完整的评估执行流程和结果解读表。最后介绍在线 RAG 应用平台（Dify/FastGPT/LangSmith/RAGFlow）以及使用平台与手写代码的选型建议。
