...
> 📝 **文档摘要：** 本文讲解 RAG 从基础到高级架构的演进。首先分析朴素 RAG 的痛点（检索质量不稳定、用户提问不精准、检索结果太多），引出高级 RAG 的核心思路——在检索"前"和检索"后"各加一道优化工序。然后详细介绍前索引（问题重写/问题扩展/假设性问题）和后索引（重排序/过滤/压缩）的概念。重点讲解五种高级 RAG 架构：T-RAG（树状检索）、C-RAG（纠正性检索，含重试策略和优先队列）、Self-RAG（自我反思，含三种重试策略）、Fusion RAG（多问题融合检索）、Rewrite RAG（问题重写+奖励机制）。每种架构都有完整的伪代码实现和适用场景分析。最后对比 RAG 与微调的差异，并提供选型建议和混合架构实战示例。
