...
$$
\text{CrossEntropy} = -\frac{1}{N} \sum_{i=1}^{N} \log\big(p_i[y_i]\big)
$$

其中：
- $y_i$ = 第 i 个样本的**真实类别编号**（如 0, 1, 2...）
- $p_i[y_i]$ = 模型预测该样本属于类别 $y_i$ 的概率
...
| 损失函数 | 别名 | 公式 | 特点 |
|---------|:---:|------|------|
| **MSE**（均方误差） | L2 Loss | $\displaystyle\frac{1}{N}\sum (y - \hat{y})^2$ | 光滑可导，但对异常值敏感 ❌ |
| **MAE**（平均绝对误差） | L1 Loss | $\displaystyle\frac{1}{N}\sum |y - \hat{y}|$ | 对异常值鲁棒，但零点不可导 |
| **Smooth L1** | Huber Loss 变体 | 分区间定义 | 结合二者优点 ✅ |

### 4.2 Smooth L1 公式

$$
\text{SmoothL1}(y, \hat{y}) =
\begin{cases}
0.5 (y - \hat{y})^2, & |y - \hat{y}| < 1 \\[4pt]
|y - \hat{y}| - 0.5, & |y - \hat{y}| \geq 1
\end{cases}
$$
- 小误差时 → 用 L2（光滑可导）
- 大误差时 → 用 L1（降低异常值影响）
