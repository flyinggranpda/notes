...
解码器当前状态:  hₜ            (hidden_size,)
  └─ hₜ 是解码器 RNN 在当前时间步 t 的隐状态，
     编码了"到目前为止已经输出/翻译了什么内容"。
     同一时间步内固定，作为 Query 遍历所有 Hᵢ 算注意力；
     跨时间步随逐词输出而更新（h₁→h₂→h₃），驱动注意力"移动"。
     初始状态 h₀ 来自编码器最终状态，但之后各自独立更新。

     注意对比：hₜ（解码器状态，处理输出序列）≠ Hᵢ（编码器状态，处理输入序列），
     两者来自不同 RNN、身处不同语义空间，即使编号相同也不相等。
